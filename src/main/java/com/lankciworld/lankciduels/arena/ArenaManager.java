...
public class ArenaManager {
    ...
    public synchronized void load() {
        // Keep a reference to the arenas that existed before this reload -
        // see the reuse step below for why this matters.
        Map<String, Arena> previous = new LinkedHashMap<>(arenas);
        arenas.clear();
        ...
        for (String name : section.getKeys(false)) {
            ...
            Location spawn1 = readLocation(arenaSection.getConfigurationSection("spawn1"), worldName);
            Location spawn2 = readLocation(arenaSection.getConfigurationSection("spawn2"), worldName);

            Arena existing = previous.get(key);
            if (existing != null) {
                // Reuse the SAME Arena instance instead of creating a new
                // one. Any Duel currently in progress on this arena holds a
                // direct reference to this exact object (Duel#getArena()),
                // including its runtime status/activeDuelId - replacing it
                // with a fresh Arena here would silently reset that arena
                // back to FREE in this map while the old (now orphaned)
                // instance still says IN_USE, letting a second duel be
                // booked onto the same physical arena mid-reload. Only the
                // on-disk config (world/spawn points) is refreshed; status
                // and activeDuelId are left untouched.
                existing.setWorldName(worldName);
                existing.setSpawn1(spawn1);
                existing.setSpawn2(spawn2);
                arenas.put(key, existing);
            } else {
    ...
    public static class UnresolvedLocation extends Location {
        ...
        /**
         * The base {@link Location#clone()} builds its copy from the raw
         * (private) world field rather than calling {@link #getWorld()},
         * so without this override a clone of an UnresolvedLocation would
         * silently end up with a permanently null world instead of the
         * same lazy resolution behaviour - defeating the entire point of
         * this class. Overriding it here keeps any current or future
         * {@code .clone()} call on an arena spawn point safe.
         */
        @Override
        public UnresolvedLocation clone() {
            return new UnresolvedLocation(worldName, getX(), getY(), getZ(), getYaw(), getPitch());
        }
    }
}
