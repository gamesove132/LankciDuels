...
public class DuelCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleStats(CommandSender sender, String[] args) {
        ...
        if (args.length >= 2) {
            ...
            if (online != null) {
                showStats(sender, online);
                return;
            }
            // See DuelAdminCommand#handleForceStop for why an offline-name
            // lookup is resolved off the main thread rather than calling
            // the deprecated Bukkit.getOfflinePlayer(String) directly here.
            plugin.async(() -> {
                OfflinePlayer offline = Bukkit.getOfflinePlayer(targetName);
                plugin.sync(() -> showStats(sender, offline));
            });
            return;
        }
        if (sender instanceof Player player) {
