...
public class PlayerListener implements Listener {
    ...
    // Purely internal anti-spam for the boundary-hit message below - not a
    // gameplay setting, so it isn't exposed in config.yml.
    private static final long BOUNDARY_MESSAGE_COOLDOWN_MS = 2000L;
    private final CooldownManager boundaryMessageCooldown = new CooldownManager();
    ...
    public void onMove(PlayerMoveEvent event) {
        ...
        // Keeps an actively-fighting duelist from flying/falling out of
        // their own arena. Checked before the countdown-freeze branch
        // below since it covers a different state (FIGHTING) and a
        // different concern (leaving the arena vs. not moving at all).
        if (duel != null && duel.getState() == DuelState.FIGHTING
                && enforceArenaBoundary(event, player, duel)) {
            return;
        }
    ...
    /**
     * Acts like an invisible wall around the arena: if the duelist's move
     * would take them past the configured radius, or drop them too far
     * below the arena's spawn height (e.g. into the void on an elevated
     * platform arena), the move is reverted instead of allowed through.
     * This is only a safety net on top of whatever physical walls/fences
     * the arena is built with - returns true if it handled (reverted)
     * this move.
     */
    private boolean enforceArenaBoundary(PlayerMoveEvent event, Player player, Duel duel) {
        if (!plugin.getConfigManager().arenaBoundaryEnabled()) {
            return false;
        }
        ...
        double radius = plugin.getConfigManager().arenaBoundaryRadius();
        boolean outsideRadius = !arena.isInsideRadius(to, radius);

        double floorY = Math.min(spawn1.getY(), spawn2.getY()) - plugin.getConfigManager().arenaBoundaryMaxFall();
        boolean belowFloor = to.getWorld() != null && spawn1.getWorld() != null
                && to.getWorld().equals(spawn1.getWorld()) && to.getY() < floorY;

        if (!outsideRadius && !belowFloor) {
            return false;
        }

        event.setTo(event.getFrom());
        if (!boundaryMessageCooldown.isOnCooldown(player.getUniqueId(), BOUNDARY_MESSAGE_COOLDOWN_MS)) {
            boundaryMessageCooldown.trigger(player.getUniqueId());
            plugin.getMessages().send(player, "arena-boundary-hit");
        }
        return true;
    }
