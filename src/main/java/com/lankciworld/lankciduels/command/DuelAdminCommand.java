...
public class DuelAdminCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleForceStop(CommandSender sender, String[] args) {
        ...
        if (online != null) {
            finishForceStop(sender, online, targetName);
            return;
        }
        // Bukkit.getOfflinePlayer(String) is deprecated in part because
        // resolving a name that was never online can involve extra local
        // lookup work - resolve it on the async scheduler instead of the
        // main thread, then hop back with plugin.sync(...) before touching
        // DuelManager#forceStop (which teleports/restores live players and
        // must run on the main thread).
        plugin.async(() -> {
            OfflinePlayer offlineTarget = Bukkit.getOfflinePlayer(targetName);
            plugin.sync(() -> finishForceStop(sender, offlineTarget, targetName));
        });
    }

    private void finishForceStop(CommandSender sender, OfflinePlayer target, String targetName) {
        boolean stopped = plugin.getDuelManager().forceStop((sender instanceof Player p) ? p : null, target);
        if (stopped) {
            plugin.getMessages().send(sender, "duel-forcestopped-admin", Map.of("player", targetName));
        } else {
